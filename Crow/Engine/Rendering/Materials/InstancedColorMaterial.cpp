...
#include "InstancedColorMaterial.h"
#include "../Model.h"
...
InstancedColorMaterial::InstancedColorMaterial() :  AbstractMaterial("instancedLitShader")
{
    Initialize();
}

void InstancedColorMaterial::BufferShaderUniforms(const glm::mat4 &pViewMatrix, const glm::mat4 &pPerspectiveMatrix,
                                                  const glm::vec3 &viewPos, World *world)
{
    AbstractMaterial::BufferShaderUniforms(pViewMatrix, pPerspectiveMatrix, viewPos, world);

    if(m_shader->bufferedThisFrame == true) return;
...
void InstancedColorMaterial::BufferMaterialUniforms()
{
